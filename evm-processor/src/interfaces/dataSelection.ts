...
type LogType<R, A = string> = (R extends true
    ? EvmLog
    : R extends LogRequest ? LogFields<R> : never)
    & {address: A}
...
export type LogData<R extends LogDataRequest = {log: true}, A = string>
    = WithProp<'log', LogType<R['log'], A>>
...
export type TransactionData<R extends TransactionDataRequest = {transaction: true}> =
    WithProp<"transaction", TransactionType<R["transaction"]>>


type SetAddress<T, A> = Omit<T, "address"> & {address: A}
type SetItemAddress<T, P, A> = P extends keyof T
    ? Omit<T, P> & {[p in P]: SetAddress<T[P], A>} & {address: A}
    : never
...
export type LogItem<Address, R = false> = WithKind<
    "log",
    SetItemAddress<
        R extends true ? LogData<{log: true}, Address> : R extends LogDataRequest ? LogData<R, Address> : LogData<{log: {}}, Address>,
        'log',
        Address
    >
>
...
export type TransactionItem<Address, R = false> = WithKind<
    "transaction",
    SetItemAddress<
        R extends true ? TransactionData : R extends TransactionDataRequest ? TransactionData<R> : TransactionData<{transaction: {}}>,
        'log',
        Address
    >
>
